...
function buildCognitoUserAndIdentityPools(event, cb) {
  ...
  var project = event.ResourceProperties.AdditionalData.project;

  // CREATE USER POOL PARAMS
  var params = {
    PoolName: project + environment,
    AliasAttributes: [
      'email',
    ],
  ...
  cognitoidentityserviceprovider.createUserPool(params, function(err, data) {
    if (err) {
      ...
      return cb('ERROR IN CREATING USER POOL', err);
    } else {
      console.log('CREATED USER POOL', data);
      ...
      cognitoidentityserviceprovider.createUserPoolClient(params, function(err, data) {
        if (err) {
          console.log('ERROR IN CREATING USER POOL CLIENT', err);
          return cb(err);
        } else {
          console.log('CREATED USER POOL CLIENT', data);
          ...
          cognitoidentity.createIdentityPool(params, function(err, data) {
            if (err) {
              console.log('ERROR IN CREATING IDENTITY POOL', err);
              return cb(err);
            } else {
              console.log('CREATED IDENTITY POOL', data);
              return cb(null, data);
            }
          });
        }
