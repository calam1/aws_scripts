module.exports.handler = function(event, context) {
  ...
  buildCognitoUserAndIdentityPools(event, context);
}
...
function buildCognitoUserAndIdentityPools(event, context) {
  ...
  // CREATE USER POOL 
  cognitoidentityserviceprovider.createUserPool(event, context, params, function(err, data) {
    if (err) {
      console.log(err, err.stack);
      return sendResponse(event, context, "FAILURE", null, err)
    } else {
      console.log(data);
      ...
      cognitoidentityserviceprovider.createUserPoolClient(event, context, params, function(err, data) {
        if (err) {
          console.log(err, err.stack);
          return sendResponse(event, context, "FAILURE", null, err)
        } else {
          ...
          cognitoidentity.createIdentityPool(event, context, params, function(err, data) {
            if (err) {
              console.log(err, err.stack);
              return sendResponse(event, context, "FAILURE", null, err)
            } else {
